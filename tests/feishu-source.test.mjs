...
test('feishu doc url matcher and token parser work', () => {
  const docxUrl = 'https://foodtalks.feishu.cn/docx/UMsDdOwGNoUww0x6G1VcWoSLnMd?from=from_copylink';
  const wikiUrl = 'https://bytedance.larkoffice.com/wiki/UfQLwGX3piowJUkYgKac5Un1nuz';
  assert.equal(isFeishuDocUrl(docxUrl), true);
  assert.equal(parseFeishuDocToken(docxUrl), 'UMsDdOwGNoUww0x6G1VcWoSLnMd');
  assert.equal(isFeishuDocUrl(wikiUrl), true);
  assert.equal(parseFeishuDocToken(wikiUrl), 'UfQLwGX3piowJUkYgKac5Un1nuz');
...
test('extractFeishuDocByApi renders text/link/image blocks in order', async () => {
  ...
  assert.equal(result.contentHtml.includes('<h2><span class="feishu-heading-seq">1.1</span> 二级标题</h2>'), true);
...
test('extractFeishuDocByApi injects heading sequence when heading has no explicit number', async () => {
  const docToken = 'DOCTOKENHEADSEQ001';
        ...
        data: {
          ...
          items: [
            ...
            },
            {
              ...
              block_type: 4,
              heading2: {
                elements: [{ text_run: { content: '入口', text_element_style: {} } }]
              }
            },
            {
              block_id: 'h2manual',
              block_type: 4,
              heading2: {
                elements: [{ text_run: { content: '2.3 手工编号标题', text_element_style: {} } }]
              }
            }
          ]
        }
  ...
  assert.equal(result.contentHtml.includes('<h2><span class="feishu-heading-seq">1.1</span> 入口</h2>'), true);
...
test('extractFeishuDocByApi renders complex blocks: list/table/grid/quote/iframe', async () => {
  ...
  assert.equal(result.contentHtml.includes('style="width:100%;table-layout:fixed;"'), true);
  assert.equal(result.contentHtml.includes('<col style="width:37.5%;" />'), true);
  assert.equal(result.contentHtml.includes('<col style="width:62.5%;" />'), true);
  ...
  assert.equal(result.contentHtml.includes('grid-template-columns:40fr 60fr'), true);
...
test('extractFeishuDocByApi merges multi-line image captions without swallowing body paragraph', async () => {
  const docToken = 'DOCWITHMULTICAP001';
    ...
    if (textUrl.includes(`/docx/v1/documents/${docToken}/blocks`)) {
      return createJsonResponse({
        ...
        data: {
          ...
          page_token: '',
          items: [
            { block_id: docToken, block_type: 1, children: ['img1', 'cap1', 'cap2', 'p1'] },
            {
              block_id: 'img1',
              block_type: 27,
              image: { token: 'caption_image_token', align: 2 }
            },
            {
              ...
              block_type: 2,
              text: {
                elements: [{ text_run: { content: '「慕酪斯特」小红书评论', text_element_style: {} } }],
                style: { align: 2 }
              }
            },
            {
              ...
              block_type: 2,
              text: {
                elements: [{ text_run: { content: '图片来源：小红书', text_element_style: {} } }],
                style: { align: 2 }
              }
            },
            {
              ...
              text: {
                elements: [
                  {
                    text_run: {
                      content: '根据小红书评论，生布丁在口感和香味上有显著区别。',
                      text_element_style: {}
                    }
                  }
                ],
  ...
  assert.equal(result.images.length, 1);
  assert.equal(result.images[0].caption.includes('「慕酪斯特」小红书评论'), true);
  assert.equal(result.images[0].caption.includes('图片来源：小红书'), true);
  assert.equal(result.contentHtml.includes('<figcaption style="text-align:center;"><p style="text-align:center;">「慕酪斯特」小红书评论</p><p style="text-align:center;">图片来源：小红书</p></figcaption>'), true);
  assert.equal(result.contentHtml.includes('根据小红书评论，生布丁在口感和香味上有显著区别。'), true);
});
